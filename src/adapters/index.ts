 ...
 * 2. MD endpoint adapter (if supported or --use-md-endpoint)
 * 3. Site-specific adapter (MDX for Claude/Vercel)
 * 4. Playwright + turndown
 *
 * --use-jina mode:
 * 1. GitHub adapter
 * 2. MD endpoint adapter (if supported or --use-md-endpoint)
 * 3. Site-specific adapter
 * 4. Jina Reader API
...
export async function fetchWithFallback(
  ...
  // 1. Try GitHub adapter (highest priority for GitHub URLs)
  ...
  // 2. Try MD endpoint adapter (if enabled or auto-detected)
  ...
  // 3. Try site-specific adapter (MDX)
  ...
  // 4. Try Jina Reader if enabled (opt-in)
  if (options.useJina) {
    try {
      if (options.verbose) {
  ...
  // 5. Playwright adapter (default/fallback)
  try {
    if (options.verbose) {
